...
const Navbar: React.FC<NavbarProps> = ({ user, currentView, onNavigate, onLogout, isOpen, onClose }) => {
  return (
    <>
      ...
      <nav className={`fixed md:sticky top-0 left-0 h-screen w-80 bg-[var(--bg-secondary)] border-r border-black/5 flex flex-col p-10 z-[70] shrink-0 transition-all duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        {/* Mobile Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col gap-4 cursor-pointer mb-20 group" onClick={() => { onNavigate('search'); onClose(); }}>
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span className="text-3xl font-black text-[var(--text-main)] tracking-[0.2em]">PR</span>
              <div className="relative">
                <span className="text-3xl font-black text-[var(--text-main)] tracking-[0.2em] opacity-0">O</span>
              ...
              <span className="text-3xl font-black text-[var(--text-main)] tracking-[0.2em]">PERLY</span>
            </div>
        ...
        <div className="pt-10 border-t border-black/5">
          <div 
            onClick={() => { onNavigate('profile'); onClose(); }}
            className="flex items-center gap-5 p-5 bg-[var(--bg-main)] rounded-3xl border border-black/5 shadow-md cursor-pointer hover:bg-[var(--bg-secondary)] transition-all hover:scale-[1.02] active:scale-[0.98] group"
          >
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-xs font-black text-[var(--text-main)] truncate uppercase tracking-tight group-hover:text-[#00AEEF] transition-colors">{user.name}</span>
